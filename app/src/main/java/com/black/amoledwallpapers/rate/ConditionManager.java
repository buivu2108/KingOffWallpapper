...
public class ConditionManager {
    ...
    private void setFirstLaunchDay() {
        SharedPreferences preferences = context.getSharedPreferences(RateConst.SHARED_PREFERENCES, 0);
        long firstLaunchDay = preferences.getLong(RateConst.SHARED_PREFERENCES_DAYS, -1);
        if (firstLaunchDay == -1) {
            preferences.edit().putLong(RateConst.SHARED_PREFERENCES_DAYS, System.currentTimeMillis()).apply();
        }
    }
    ...
    private boolean daysToWaitReached(int daysToWait) {
        long firstLaunchDay = context.getSharedPreferences(RateConst.SHARED_PREFERENCES, 0).getLong(RateConst.SHARED_PREFERENCES_DAYS, -1);
    ...
    private boolean timesToLaunchReached(int timesToLaunch, int timesToLaunchInterval) {
        long timesLaunched = context.getSharedPreferences(RateConst.SHARED_PREFERENCES, 0).getLong(RateConst.SHARED_PREFERENCES_LAUNCHES, 0);
    ...
    private boolean showDialog() {
        return context.getSharedPreferences(RateConst.SHARED_PREFERENCES, 0).getBoolean(RateConst.SHARED_PREFERENCES_SHOW, true);
    }
    ...
    void refreshConditions() {
        long numberOfLaunches = context.getSharedPreferences(RateConst.SHARED_PREFERENCES, 0).getLong(RateConst.SHARED_PREFERENCES_LAUNCHES, 0);
        context.getSharedPreferences(RateConst.SHARED_PREFERENCES, 0).edit().putLong(RateConst.SHARED_PREFERENCES_LAUNCHES, numberOfLaunches + 1).apply();
    }

    void dontShowDialogAgain() {
        context.getSharedPreferences(RateConst.SHARED_PREFERENCES, 0).edit().putBoolean(RateConst.SHARED_PREFERENCES_SHOW, false).apply();
    }
}
